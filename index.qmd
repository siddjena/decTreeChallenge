...
The Consensus: Categorical is Key. It's essential to model zip codes as a categorical feature, not numerical. While they look like numbers, they're just labels for distinct geographic areas—they don't represent a quantity, distance, or meaningful rank. Treating them numerically forces the model to invent relationships that simply aren't there.
For example, 50012 is “greater than” 50010 in a way that matters for housing prices. In reality, the values are just identifiers.

**Is zip code ordinal or non-ordinal?**

Ordinality Check: Zip codes are definitively non-ordinal. A higher number (say, 50013) doesn't imply a higher price or better neighborhood than a lower number (50010). There's no inherent order related to the outcome.

**Why does this matter for feature importance?**

When a tree treats zip code numerically, it creates nonsensical splits like zipCode <= 50012.5. The model starts grouping neighborhoods based on the first few digits, which has zero real-world meaning for housing prices. This artificial grouping leads to misleading feature importance: the true impact of neighborhood quality gets lost, or the model ends up valuing a meaningless numerical pattern instead. Proper categorical encoding (like one-hot) allows the tree to split on meaningful questions—e.g., "Is the house in neighborhood A?"

### Question 2: R vs Python Implementation Differences

**The Critical Difference**

The Critical Difference: Native Handling vs. Manual Encoding The gap between R and Python's decision tree implementations comes down to how they handle categorical variables natively. This isn't just a minor technicality; it profoundly affects both model efficiency and the resulting interpretability.

**What R's `rpart` offers that Python's `sklearn` doesn't:**

R’s rpart package supports categorical variables natively. When given a factor variable, the algorithm can test all possible groupings of categories when choosing a split. For four zip codes, rpart can consider splits such as:

*{50010, 50011} vs {50012, 50013}

*{50010} vs {50011, 50012, 50013} and many other combinations

This allows the tree to find the most meaningful way to group neighborhoods based on their relationship to housing prices.

In contrast, scikit-learn’s DecisionTreeRegressor does not support categorical variables directly. Zip codes must be manually encoded, usually through one-hot encoding. While this avoids the problem of treating them as numeric, it limits the tree to splitting on one category at a time. It cannot create grouped splits the way rpart can, and this can reduce interpretability and model efficiency.

**Which implementation is better?**

For categorical variables, R's rpart handles them more naturally. It produces cleaner trees, more accurate feature importance, and can make more efficient splits because it evaluates category subsets directly.

Documentation from scikit-learn:
Scikit-learn’s documentation explicitly acknowledges the limitation: "Able to handle both numerical and categorical data. However, the scikit-learn implementation does not support categorical variables for now."

This is a significant constraint when working with datasets that rely heavily on categorical predictors.

### Question 3: Better Categorical Handling in Python

Since scikit-learn does not natively support categorical features in trees, other Python libraries have emerged to handle this better.

**CatBoost:**

CatBoost is built specifically to work with categorical variables without manual encoding. Its documentation notes:

"CatBoost attempts to solve for categorical features using a permutation-driven alternative to the classical algorithm."

CatBoost automatically handles categorical variables using ordered target statistics, making it well-suited for problems like modeling neighborhood effects.

**LightGBM:**

LightGBM also supports categorical variables, though it requires ordinal encoding before training. Once encoded, LightGBM treats categories using an internal ordering strategy that avoids the usual numeric pitfalls.

A common summary from practitioners is:

"CatBoost does on-the-fly target encoding, while LightGBM needs ordinal encoding."

Both methods avoid strict reliance on one-hot encoding and scale well to larger datasets.

**StructureBoost:**

StructureBoost is a newer library aimed at categorical variables with more complex structure. An arXiv paper notes:

StructureBoost outperforms established packages such as CatBoost and LightGBM on problems with categorical predictors that contain sophisticated structure.

Practical Recommendation

For most practical applications, CatBoost is the easiest and most reliable way to model categorical variables in Python. It integrates smoothly with common workflows and avoids the limitations of one-hot encoding.
