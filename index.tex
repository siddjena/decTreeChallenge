...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, left=2mm, colback=white, colframe=quarto-callout-warning-color-frame, titlerule=0mm, rightrule=.15mm, opacitybacktitle=0.6, colbacktitle=quarto-callout-warning-color!10!white, breakable, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{⚠️ AI Partnership Required}, opacityback=0, bottomtitle=1mm, toptitle=1mm, leftrule=.75mm, coltitle=black, arc=.35mm, toprule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, left=2mm, colback=white, colframe=quarto-callout-important-color-frame, titlerule=0mm, rightrule=.15mm, opacitybacktitle=0.6, colbacktitle=quarto-callout-important-color!10!white, breakable, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{🎯 The Key Insight: Encoding Matters for Interpretability}, opacityback=0, bottomtitle=1mm, toptitle=1mm, leftrule=.75mm, coltitle=black, arc=.35mm, toprule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, left=2mm, colback=white, colframe=quarto-callout-warning-color-frame, titlerule=0mm, rightrule=.15mm, opacitybacktitle=0.6, colbacktitle=quarto-callout-warning-color!10!white, breakable, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{⚠️ The Problem Revealed}, opacityback=0, bottomtitle=1mm, toptitle=1mm, leftrule=.75mm, coltitle=black, arc=.35mm, toprule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, left=2mm, colback=white, colframe=quarto-callout-important-color-frame, titlerule=0mm, rightrule=.15mm, opacitybacktitle=0.6, colbacktitle=quarto-callout-important-color!10!white, breakable, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{📁 Quick Start with Starter Repository}, opacityback=0, bottomtitle=1mm, toptitle=1mm, leftrule=.75mm, coltitle=black, arc=.35mm, toprule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, left=2mm, colback=white, colframe=quarto-callout-tip-color-frame, titlerule=0mm, rightrule=.15mm, opacitybacktitle=0.6, colbacktitle=quarto-callout-tip-color!10!white, breakable, title=\textcolor{quarto-callout-tip-color}{\faLightbulb}\hspace{0.5em}{💡 Why Use the Starter Repository?}, opacityback=0, bottomtitle=1mm, toptitle=1mm, leftrule=.75mm, coltitle=black, arc=.35mm, toprule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, left=2mm, colback=white, colframe=quarto-callout-note-color-frame, titlerule=0mm, rightrule=.15mm, opacitybacktitle=0.6, colbacktitle=quarto-callout-note-color!10!white, breakable, title=\textcolor{quarto-callout-note-color}{\faInfo}\hspace{0.5em}{🎯 Navy SEALs Motto}, opacityback=0, bottomtitle=1mm, toptitle=1mm, leftrule=.75mm, coltitle=black, arc=.35mm, toprule=.15mm]
...
\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, left=2mm, colback=white, colframe=quarto-callout-warning-color-frame, titlerule=0mm, rightrule=.15mm, opacitybacktitle=0.6, colbacktitle=quarto-callout-warning-color!10!white, breakable, title=\textcolor{quarto-callout-warning-color}{\faExclamationTriangle}\hspace{0.5em}{💾 Important: Save Your Work Frequently!}, opacityback=0, bottomtitle=1mm, toptitle=1mm, leftrule=.75mm, coltitle=black, arc=.35mm, toprule=.15mm]
...
\subsection{Discussion Questions - Answers
📝}\label{discussion-questions---answers}

\subsubsection{Question 1: Numerical vs Categorical
Encoding}\label{question-1-numerical-vs-categorical-encoding}

\textbf{How should zip code be modeled?}

Zip codes should be treated as categorical, not numerical. Although they
look like numbers, they label geographic areas. The digits do not
represent quantities, distances, or any meaningful ordering.

Treating zip codes numerically implies relationships that do not exist.
For example, 50012 is ``greater than'' 50010 in a way that matters for
housing prices. In reality, the values are just identifiers.

\textbf{Is zip code ordinal or non-ordinal?}

Zip codes are non-ordinal categorical variables. There is no natural
ranking among them. A higher or lower number does not correspond to a
``better'' or ``worse'' neighborhood. Each zip code represents a
distinct area with characteristics unrelated to its numerical value.

\textbf{Why does this matter for feature importance?}

A decision tree using zip code as a numeric variable creates splits like
zipCode \textless= 50012.5, which have no real-world interpretation.
These splits group neighborhoods based on their numbers rather than on
actual similarities. As a result, feature importance can be
misleading---either undervaluing the true impact of neighborhood or
detecting patterns that are purely artifacts of the numeric encoding.

When zip code is encoded properly as a categorical variable (as, one-hot
encoding in Python), the tree can split on meaningful questions such as
whether a house is located in a particular neighborhood. This leads to
more interpretable and accurate results.

\subsubsection{Question 2: R vs Python Implementation
Differences}\label{question-2-r-vs-python-implementation-differences}

\textbf{The Critical Difference}

The fundamental distinction between R and Python implementations of
decision trees lies in their native handling of categorical
variables---and this difference is neither subtle nor academic. It
directly impacts both model performance and interpretability.

\textbf{What R's \texttt{rpart} offers that Python's \texttt{sklearn}
doesn't:}

R's rpart package supports categorical variables natively. When given a
factor variable, the algorithm can test all possible groupings of
categories when choosing a split. For four zip codes, rpart can consider
splits such as:

*\{50010, 50011\} vs \{50012, 50013\}

*\{50010\} vs \{50011, 50012, 50013\} and many other combinations

This allows the tree to find the most meaningful way to group
neighborhoods based on their relationship to housing prices.

In contrast, scikit-learn's DecisionTreeRegressor does not support
categorical variables directly. Zip codes must be manually encoded,
usually through one-hot encoding. While this avoids the problem of
treating them as numeric, it limits the tree to splitting on one
category at a time. It cannot create grouped splits the way rpart can,
and this can reduce interpretability and model efficiency.

\textbf{Which implementation is better?}

For categorical variables, R's rpart handles them more naturally. It
produces cleaner trees, more accurate feature importance, and can make
more efficient splits because it evaluates category subsets directly.

Documentation from scikit-learn: Scikit-learn's documentation explicitly
acknowledges the limitation: ``Able to handle both numerical and
categorical data. However, the scikit-learn implementation does not
support categorical variables for now.''

This is a significant constraint when working with datasets that rely
heavily on categorical predictors.

\subsubsection{Question 3: Better Categorical Handling in
Python}\label{question-3-better-categorical-handling-in-python}

Since scikit-learn does not natively support categorical features in
trees, other Python libraries have emerged to handle this better.
...
CatBoost is built specifically to work with categorical variables
without manual encoding. Its documentation notes:

*CatBoost attempts to solve for categorical features using a
permutation-driven alternative to the classical algorithm.

CatBoost automatically handles categorical variables using ordered
target statistics, making it well-suited for problems like modeling
...
LightGBM also supports categorical variables, though it requires ordinal
encoding before training. Once encoded, LightGBM treats categories using
an internal ordering strategy that avoids the usual numeric pitfalls.

A common summary from practitioners is:

CatBoost does on-the-fly target encoding, while LightGBM needs ordinal
encoding.

Both methods avoid strict reliance on one-hot encoding and scale well to
larger datasets.
...
StructureBoost is a newer library aimed at categorical variables with
more complex structure. An arXiv paper notes:

StructureBoost outperforms established packages such as CatBoost and
LightGBM on problems with categorical predictors that contain
sophisticated structure.

Practical Recommendation

For most practical applications, CatBoost is the easiest and most
reliable way to model categorical variables in Python. It integrates
smoothly with common workflows and avoids the limitations of one-hot
encoding.
...
\NormalTok{model }\OperatorTok{=}\NormalTok{ CatBoostRegressor(}
\NormalTok{    iterations}\OperatorTok{=}\DecValTok{100}\NormalTok{,}
\NormalTok{    depth}\OperatorTok{=}\DecValTok{3}\NormalTok{,}
\NormalTok{    cat\_features}\OperatorTok{=}\NormalTok{[}\StringTok{\textquotesingle{}zipCode\textquotesingle{}}\NormalTok{]}
\NormalTok{)}
...
No manual encoding, no one-hot expansion, no loss of
interpretability---just cleaner, more accurate modeling of categorical
relationships.

\subsection{Grading Rubric 🎓}\label{grading-rubric}

\begin{tcolorbox}[enhanced jigsaw, bottomrule=.15mm, left=2mm, colback=white, colframe=quarto-callout-important-color-frame, titlerule=0mm, rightrule=.15mm, opacitybacktitle=0.6, colbacktitle=quarto-callout-important-color!10!white, breakable, title=\textcolor{quarto-callout-important-color}{\faExclamation}\hspace{0.5em}{📊 What You're Really Being Graded On}, opacityback=0, bottomtitle=1mm, toptitle=1mm, leftrule=.75mm, coltitle=black, arc=.35mm, toprule=.15mm]
